...
void main() {
  ...
  if (galleryPageGrid) {
    ...
    const renderGallery = (images) => {
      ...
      // Set up global observer for progressive loading & revealing
      ...
      const createGalleryItem = (url, index, allImages) => {
        ...
        const img = document.createElement('img');
        img.src = fullUrl; // Load the high-res image directly to prevent blurry loading
      ...
      const distributeItems = () => {
        let colsCount = 4;
        if (window.innerWidth <= 768 || document.documentElement.classList.contains('is-mobile')) {
          colsCount = 2;
    ...
    likeContainers.forEach(async (container) => {
      ...
      if (commentForm) {
        ...
        commentForm.addEventListener('submit', (e) => {
          ...
          const emailEl = commentForm.querySelector('.story-comment-email');
