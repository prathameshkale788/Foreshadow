...
const init = () => {
  ...
  if (cursor && cursorDot && window.innerWidth > 768) {
    ...
    const hoverTargets = document.querySelectorAll('a, button, .portfolio-item, .portfolio-card, .story-entry, .portfolio-filter-btn, .film-card, .blog-card, .service-card, .masonry-item, .iconic-cell, .camera-photo-card, .nav-hamburger, .btn-vmpf, .carousel-slide, .aesthetic-slide, .aesthetic-slider-arrow');
  ...
  if (galleryPageGrid) {
    ...
    try {
      const stored = localStorage.getItem('gallery_image_dimensions_cache');
      if (stored) {
        dynamicCache = JSON.parse(stored);
      }
    } catch (err) { console.warn('Could not read dynamic cache:', err); }

    // Optimized image dimension check with static pre-calculated lookup and dynamic local cache fallback
    const getImageDimensions = (url) => {
      // 1. Check pre-calculated static map
      if (url in dimensionsMap) {
        return Promise.resolve({ url, isPortrait: dimensionsMap[url] });
      }
      // 2. Check dynamic cache
      if (url in dynamicCache) {
        return Promise.resolve({ url, isPortrait: dynamicCache[url] });
      }
      // 3. Fallback to natural image parsing
      return new Promise((resolve) => {
        ...
        img.onload = () => {
          ...
          dynamicCache[url] = isPort;
          try {
            localStorage.setItem('gallery_image_dimensions_cache', JSON.stringify(dynamicCache));
          } catch (err) {}
          resolve({ url, isPortrait: isPort });
        };
        img.onerror = () => {
          clearTimeout(timeout);
          resolve({ url, isPortrait: false });
    ...
    // Fetch dimensions mapping in parallel with gallery images to optimize load times
    Promise.all([
      fetch('js/gallery_dimensions.json')
      ...
      console.warn('Error fetching gallery data, falling back to local list:', err);
      if (localImages.length > 0) {
        renderGallery(localImages);
      } else {
        if (loadingEl) loadingEl.textContent = 'Failed to load gallery.';
      }
    });
  }
